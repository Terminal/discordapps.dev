...
const schema = joi.object({
  ...
  authors: joi.array().items(joi.string().regex(/^[0-9]+$/, 'numbers').min(1).max(10)).required().error(new Error('bot_authors_error')),
  github: joi.object({
  ...
  trigger: joi.object({
    prefix: joi.array().items(joi.string().min(1).max(10)).min(1).max(10).error(new Error('bot_prefix_error')),
  ...
  contents: joi.object().pattern(joi.string().valid(i18n.getLocales()), joi.object({
    name: joi.string().min(4).max(32).required().error(new Error('bot_name_error')),
    description: joi.string().min(10).max(64).required().error(new Error('bot_description_error')),
    page: joi.string().min(20).max(10000).required().error(new Error('bot_page_error')),
